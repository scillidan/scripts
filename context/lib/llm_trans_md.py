...
#
# Built-in defaults:
#   LLM_TRANS_MODEL=gemma-3-12b-it
#   LLM_TRANS_SRC=en
#   LLM_TRANS_TGT=zh
...
_RE_FULL_PUNCT_SPACE = re.compile(
    r"([{}\u4e00-\u9fff\u3400-\u4dbf])\s+([{}\u4e00-\u9fff\u3400-\u4dbf])".format(
        _FULL_PUNCT, _FULL_PUNCT
    )
)
...
def postprocess(text):
    ...
    text = _RE_PUNCT_AFTER_CJK.sub(
        lambda m: m.group(1) + _HALF_TO_FULL[m.group(2)], text
    )
    text = _RE_PUNCT_BEFORE_CJK.sub(
        lambda m: _HALF_TO_FULL[m.group(1)] + m.group(2), text
    )
    text = _RE_PAREN_AFTER_CJK.sub(
        lambda m: m.group(1) + _HALF_TO_FULL[m.group(2)], text
    )
    text = _RE_PAREN_BEFORE_CJK.sub(
        lambda m: _HALF_TO_FULL[m.group(1)] + m.group(2), text
    )
...
def translate_segments(
    ...
    if use_tgemma:
        ...
        def _do_one(idx, text):
            return idx, _call_translategemma(
                text, src_lang_code, tgt_lang_code, model, host, timeout
            )
    else:
        ...
        system = (
            preset["system"]
            .replace("{src_lang}", src_label)
            .replace("{tgt_lang}", tgt_label)
        )
        user_tmpl = preset["user"]

        def _do_one(idx, text):
            prompt = (
                user_tmpl.replace("{src_lang}", src_label)
                .replace("{tgt_lang}", tgt_label)
                .replace("{text}", text)
            )
...
def translate_file(
    ...
    pp = (
        postprocess
        if (do_postprocess and is_cjk_target(tgt_lang_code))
        else (lambda x: x)
    )
...
def main():
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("LLM_TRANS_MODEL", DEFAULT_MODEL),
        help=f"Model name (default: {DEFAULT_MODEL})",
    )
    parser.add_argument(
        "--src-lang",
        default=os.environ.get("LLM_TRANS_SRC", "auto"),
        help="Source language: ISO code (zh), ISO3 (zho), name (Chinese), or auto (default: auto)",
    )
    parser.add_argument(
        "--tgt-lang",
        default=os.environ.get("LLM_TRANS_TGT", "zh"),
    ...
    if args.debug:
        model_type = (
            "TranslateGemma (/completion endpoint)"
            if is_translategemma(args.model)
            else "Standard chat (system + user prompt)"
        )
