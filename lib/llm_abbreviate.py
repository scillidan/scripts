...
#   uv run llm_abbreviate.py --host http://127.0.0.1:8080 --model gemma-3-4b-it-qat "abbreviation"
#   uv run llm_abbreviate.py --host http://127.0.0.1:8080 --model gemma-3-4b-it-qat "application programming interface"
#
#   # Read from stdin (pipe input, one word/phrase per line)
#   echo "configuration" | uv run llm_abbreviate.py --stdin
#   cat terms.txt | uv run llm_abbreviate.py --stdin --host http://localhost:8080
#
#   # With cache (persist LLM results for reuse)
#   uv run llm_abbreviate.py --host http://127.0.0.1:8080 --model gemma-3-4b-it-qat --input "abbreviation" --save-cache
#
#   # With user abbreviation table (--include entries take priority over cache and LLM)
#   uv run llm_abbreviate.py --host http://127.0.0.1:8080 --model gemma-3-4b-it-qat --input "provider" --include my_abbr.txt
...
CACHE_FILE = ".llm_abbreviate/cache.txt"
