...
# llm_comment - Generate line-by-line comments for code via llama.cpp server
# Authors: GLM-5.1🧙‍♂️, scillidan🤡
#
# Tested model: mistral-7b-instruct-v0.3
#
# Usage:
#   uv run llm_comment.py --host http://127.0.0.1:8080 --model mistral-7b-instruct-v0.3 "def foo(): pass"
#   cat code.py | uv run llm_comment.py --stdin
...
DEFAULT_PROMPT = (
    "Add concise comments to the following code explaining what each part does. "
    "Output ONLY the full code with comments added. "
    "Do not omit any lines. Do not add explanations outside the code."
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate line-by-line comments for code using local LLM."
    )
    ...
    parser.add_argument("text", nargs="*", default="", help="Input code to comment")
